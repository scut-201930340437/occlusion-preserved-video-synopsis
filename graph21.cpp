...
void find_rest_fish(std::vector<Segment*>& heads, std::vector<Segment*> fish, std::set<Segment*>& asic)
{
	std::vector<Segment*> rest_fish;
	for (size_t i = 0; i < fish.size(); i++) {
		std::vector<Segment*> path1, path2;
		bool founded;
		if (fish[i]->next_ != NULL && !fish[i]->next_->circulation_traversed) {
			path1.clear();
			path2.clear();
			...
			if (founded)
			{
				...
				{
					path1[j]->circulation_traversed = true;
					asic.insert(path1[j]);
				}

				for (size_t j = 0; j < path2.size(); j++)
				{
					path2[j]->circulation_traversed = true;
					asic.insert(path2[j]);
				}
				rest_fish.push_back(fish[i]->next_);
			}
		}
		if (fish[i]->last_ != NULL && !fish[i]->last_->circulation_traversed) {
			path1.clear();
			path2.clear();
		...
		if (fish[i]->colli_segs_.size()) {
			for (size_t k = 0; k < fish[i]->colli_segs_.size(); k++) {
				if (!fish[i]->colli_segs_[k]->circulation_traversed) {
					path1.clear();
					path2.clear();
	...
	find_rest_fish(heads, rest_fish, asic);
}


void gather_all_segments_in_circulation_by_new_method(std::vector<Segment*>& heads, Segment* A, std::set<Segment*>& asic)
{
	...
	find_rest_fish(heads, fish, asic);
}
	...
	int pos)
{
	for (size_t i = 0; i < heads.size(); i++)
	{
		...
		while (next != NULL)
		{
			if (pos >= next->a_ && pos <= next->b_ && slf->circulation_id_ != -1 && slf->circulation_id_ == next->circulation_id_)
			{
				ans.insert(next);
			}
			next = next->next_;
	...
	std::vector<Segment*>& heads, std::vector<size_t> & idx)
{
	for (size_t i = 0; i < cands.size(); i++)
	{
		...
		bool flg = check_if_two_segments_can_be_linked(heads, seg1, seg2);
		//check_if_two_segments_can_be_linked(heads, seg1, seg2);

		if (flg && !check_if_having_link_already(seg1,seg2))
...
void find_divide_points(std::vector<Segment*> & heads, 
	Segment* current, 
	std::vector<Segment*>& all_circulated_segs,
...
void resolve_circulation_problem(std::vector<Segment*> & heads)
{
	...
	for (size_t i = 0; i < heads.size(); i++)
	{
		...
		while (next != NULL)
		{
			...
			gather_all_segments_in_circulation_by_new_method(heads, next, all_circulated_segs);
			...
			for(size_t iii = 0;iii<all_circulated_segs_vec.size();iii++)
			{
				...
				current->circulation_id_ = circulation_num;
				std::cout << "(" << current->tube_id_ << "," << current->a_ << "," << current->b_ << ")" << std::endl;

				reset_last_next_break(heads);
				//find_divide_points(heads, current, all_circulated_segs_vec, divide_points, cannot_divide_points, 0);
				find_divide_points(heads, current, all_circulated_segs_vec, divide_points, 0);
				reset_last_next_break(heads);
				//find_divide_points(heads, current, all_circulated_segs_vec, divide_points, cannot_divide_points, 1);
				find_divide_points(heads, current, all_circulated_segs_vec, divide_points, 1);
				reset_last_next_break(heads);

				//current->circulation_traversed = true;
			}
			
			if (circulation_num == 2)
			{
				std::cout << "******cir 2**************" << std::endl;
				for (size_t iii = 0; iii < all_circulated_segs_vec.size(); iii++)
				{
					Segment* current = all_circulated_segs_vec[iii];
					std::cout << "(" << current->tube_id_ << "," << current->a_ << "," << current->b_ << ")" << std::endl;
	...
	while (true)
	{
		...
		/*for (size_t i = 0; i < inconsis_segs.size(); i++)
		{
			std::cout << inconsis_segs[i]->tube_id_ << "," << inconsis_segs[i]->a_ << "," << inconsis_segs[i]->b_ << std::endl;
		}*/
		...
		for (size_t t = 0; t < inconsis_segs.size(); t++)
		{
			...
			std::cout << "cir_id:" << inconsis_segs[t]->circulation_id_ << std::endl;
		}
		//
		...
		for (size_t t = 0; t < links.size(); t++)
		{
			std::cout << "tube1 id:" << links[t].first->tube_id_ << " " << "tube2 id:" << links[t].second->tube_id_ <<
				" beg->end:" << links[t].first->aa_<<" "<< links[t].first->bb_ << " " << links[t].second->aa_ << " " << links[t].second->bb_ <<"cir_id:"<<links[t].first->circulation_id_<< std::endl;
		}
		//


		size_t min_inconsis = 1000000;
		...
		std::vector<std::pair<Segment*, Segment*>>::iterator iter2;
